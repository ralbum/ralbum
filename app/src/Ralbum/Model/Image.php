...
class Image extends File
{
    ...
    public function updateThumbnail($updateFromDetailImage = false)
    {
        ...
        try {
            if ($updateFromDetailImage) {
                $image = new ImageProcessor(file_get_contents($this->getDetailPath()));
            } else {
                $image = new ImageProcessor(file_get_contents($this->getPath()));

                if (\Ralbum\Setting::get('auto_rotate')) {
                    $image->fixOrientation($this->getMetadata()->getOrientation());
                }
            }
    ...
    public function getPhotoAnalysis()
    {
        if (!file_exists($this->getThumbnailPath())) return null;
        $proc = new ImageProcessor(file_get_contents($this->getThumbnailPath()));
        return $proc->analyze();
    }
}
